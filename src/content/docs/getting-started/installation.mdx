...
- [SuperCollider](https://supercollider.github.io/), the backend audio server that makes the sound, with the additional [SC3 plugins](https://github.com/supercollider/sc3-plugins);
- [SuperDirt](https://github.com/musikinformatik/SuperDirt), the sample library and synth engine used by Tidal, installed in SuperCollider;
- the Tidal pattern engine (TidalCycles itself), which runs on [the Haskell Language](https://www.haskell.org/) (usually installed with [ghcup](https://www.haskell.org/ghcup/)), and which can be installed through [cabal](https://www.haskell.org/cabal/), Haskell's package manager;
- and a code editor such as [Pulsar](https://pulsar-edit.dev/), which talks to Tidal via a [plugin](https://github.com/tidalcycles/pulsar-tidalcycles).
...
<TabItem label="Windows">
  ...
  After installing Chocolatey, you can run the following command to install TidalCycles:
  ...
  {/* TODO: link to tidalcycles choco package definition and where support can be found */}
...
Make sure your user is a a member of the `audio` group:
...
Almost done! The final installation step is to choose and install a [text editor (see below)](#get-a-text-editor) to interact with TidalCycles.

</TabItem>

<TabItem label="macOS">

{/* TODO: Add links to instructions for supercollider, SC3 plugins, and ghcup */}
...
[VSCode](https://code.visualstudio.com/) is a popular general purpose text/code editor published by Microsoft, with some open-source components.
...
To send text from Helix to Tidal, you can use `tmux`: run the Tidal command line, and use the Helix "pipe to command" feature to run a `tmux` command to input the keystrokes in the Tidal command line.
...
{/* TODO: clean up upgrading, downgrading, and uninstallation sections */}
