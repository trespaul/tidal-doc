...
Installing SuperDirt will also download a [library](/reference/superdirt/default-library/samples) of audio samples. This library is known as the *default* library, the one that everybody starts with.
...
Given the folder structure above, after finishing this guide you will be able to use the `myBass`, myHits` and `myField` sounds in your Tidal code. For example:
